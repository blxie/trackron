    ...
    def track_mot(self, image, info):
        ...
        if self.frame_num > 1 and len(self.last_detect_tracks) > 0:
            ...
            target_feature = torch.tensor(
                ...
                dtype=torch.float32).to(self.device)
            self.mot_ref_info['proposal'] = normalize_boxes(
                proposal,
                image.shape[:2],
                in_format='xywh',
        ...
        outputs, self.mot_ref_info = self.net.track_mot(
    ...
    def update_mot(self, output_results, use_embedding=False):
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [
                ...
                for (tlbr, s,
                     feat) in zip(dets_second, scores_second, feats_second)
            ]
        else:
            detections_second = []
        r_tracked_stracks = [
        ...
        matches, u_track, u_detection_second = linear_assignment(dists,
                                                                 thresh=0.5)
        for itracked, idet in matches:
