    ...
    def forward(self, outputs, origin_sizes, category=None, box_fmt='cxcywh'):
        ...
        if self.box_absolute:
            scale_fct = torch.stack([
                img_w / self.w, img_h / self.h, img_w / self.w, img_h / self.h
            ],
                                    dim=1).unsqueeze(1)
        else:
        ...
        if 'tracking_boxes' in outputs:
            track_boxes = box_convert(outputs['tracking_boxes'], self.box_fmt,
                                      'xyxy')
    ...
    def get_track_scores(self, outputs, category=None):
        ...
        if outputs.get('tracking_scores', None) is not None:
            ...
            if category is None:
                ...
                track_scores, _ = track_prob[...,
                                             category:category + 1].max(-1)
        return track_scores
