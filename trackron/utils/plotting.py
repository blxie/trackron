...
def show_image_with_boxes(im, boxes, iou_pred=None, disp_ids=None):
    ...
    for i_ in range(boxes.shape[0]):
        if disp_ids is None or disp_ids[i_]:
            bb = boxes[i_, :]
            disp_color = (i_ * 38 % 256, (255 - i_ * 97) % 256,
                          (123 + i_ * 66) % 256)
            cv2.rectangle(im_np, (bb[0], bb[1]),
                          (bb[0] + bb[2], bb[1] + bb[3]), disp_color, 1)
...
def draw_box(image, box_coord, color=None, thickness=2, text=None):
    """
      Args:
          image: cv2 image 
      ...
      Returns:
          output (Image): image object with box drawn.
      """
    x0, y0, x1, y1 = map(int, box_coord)
    if color is None:
        color = _get_rand_color()
    image = cv2.rectangle(image, (x0, y0), (x1, y1), color, thickness)
    if text is not None:
        cv2.putText(image, text, (x0 + 20, y0 + 20), cv2.FONT_HERSHEY_SIMPLEX,
                    0.9, color, 5)
    return image
