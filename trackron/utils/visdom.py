...
class VisImage(VisBase):
    ...
    def draw_data(self):
        disp_image = numpy_to_torch(self.raw_data).squeeze(0)
...
class VisHeatmap(VisBase):
    ...
    def draw_data(self):
        self.visdom.heatmap(self.raw_data.clone(),
                            opts={'title': self.title},
                            win=self.title)
...
class VisFeaturemap(VisBase):
    ...
    def block_list_callback_handler(self, data):
        ...
        self.visdom.properties(self.block_list,
                               opts={'title': 'Featuremap UI'},
                               win='featuremap_ui')
        self.draw_data()
...
class VisCostVolume(VisBase):
    ...
    def show_cost_volume(self):
        ...
        self.visdom.heatmap(data_perm.flip(0),
                            opts={'title': self.title},
                            win=self.title)
    ...
    def show_cost_volume_slice(self):
        ...
        if self.flip:
            cost_volume_slice = cost_volume_data[:, :, slice_pos[0],
                                                 slice_pos[1]]
        else:
            cost_volume_slice = cost_volume_data[slice_pos[0],
                                                 slice_pos[1], :, :]
        self.visdom.heatmap(cost_volume_slice.flip(0),
                            opts={'title': self.title},
                            win=self.title)

    def save_data(self, data):
        data = data.view(data.shape[-2], data.shape[-1], data.shape[-2],
                         data.shape[-1])
        self.raw_data = data
...
class VisCostVolumeUI(VisBase):
    ...
    def cv_ui_handler(self, data):
        ...
        if data['event_type'] == 'KeyPress':
            if data['key'] == 'ArrowRight':
                self.zoom_pos[1] = min(self.zoom_pos[1] + 1,
                                       self.feat_shape[1] - 1)
                ...
                self.zoom_pos[0] = min(self.zoom_pos[0] + 1,
                                       self.feat_shape[0] - 1)
            elif data['key'] == 'Enter':
    ...
    def __init__(self, visdom, show_data, title, feat_shape,
                 registered_blocks):
        super().__init__(visdom, show_data, title)
        self.feat_shape = feat_shape
        self.zoom_mode = False
        self.zoom_pos = [
            int((feat_shape[0] - 1) / 2),
            int((feat_shape[1] - 1) / 2)
        ]
    ...
    def shade_cell(self, data):
        ...
        data[:, r1:r2,
             c1:c2] = data[:, r1:r2, c1:c2] * (1 - factor) + torch.tensor(
                 [255.0, 0.0, 0.0]).view(3, 1, 1).to(data.device) * factor
        return data
...
class VisTracking(VisBase):
    ...
    def save_data(self, data):
        ...
        boxes, masks = [], []
        if len(data) == 2:
            boxes = data[1].reshape(-1, 4)
        elif len(data) == 3:
            if len(data[2].shape) == 2:
                masks = data[2]
            elif len(data[2].shape) == 3:
                masks = [d for d in data[2]]
...
class VisBBReg(VisBase):
    ...
    def draw_data(self):
        ...
        init_box_image = show_image_with_boxes(disp_image.clone(),
                                               self.init_boxes.clone(),
                                               disp_ids=ids)
        final_box_image = show_image_with_boxes(disp_image.clone(),
                                                self.final_boxes.clone(),
                                                self.final_ious.clone(),
                                                disp_ids=ids)

        self.visdom.image(init_box_image,
                          opts={'title': 'Init Boxes'},
                          win='Init Boxes')
        self.visdom.image(final_box_image,
                          opts={'title': 'Final Boxes'},
                          win='Final Boxes')


class Visdom:

    def __init__(self, debug=0, ui_info=None, visdom_info={}):
        self.debug = debug
        self.visdom = visdom.Visdom(server=visdom_info.get(
            'server', '127.0.0.1'),
                                    port=visdom_info.get('port', 8097))
        self.registered_blocks = {}
        self.blocks_list = []

        self.visdom.properties(self.blocks_list,
                               opts={'title': 'Block List'},
                               win='block_list')
        self.visdom.register_event_handler(self.block_list_callback_handler,
                                           'block_list')

        if ui_info is not None:
            self.visdom.register_event_handler(ui_info['handler'],
                                               ui_info['win_id'])
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                self.registered_blocks[title] = VisImage(
                ...
                self.registered_blocks[title] = VisCostVolume(self.visdom,
                                                              show_data,
                                                              title,
                                                              flip=True)
            elif mode == 'cost_volume_ui':
                self.registered_blocks[title] = VisCostVolumeUI(
                    self.visdom, show_data, title, data[1],
                    self.registered_blocks)
            elif mode == 'info_dict':
                self.registered_blocks[title] = VisInfoDict(
                    self.visdom, show_data, title)
            elif mode == 'text':
                self.registered_blocks[title] = VisText(
                    self.visdom, show_data, title)
            elif mode == 'lineplot':
                ...
                raise ValueError(
                    'Visdom Error: Unknown data mode {}'.format(mode))
        # Update
