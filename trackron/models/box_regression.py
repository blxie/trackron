...
class Box2BoxTransform(object):
    ...
    def get_deltas(self, src_boxes, target_boxes):
        ...
        assert (
            src_widths >
            0).all().item(), "Input boxes to Box2BoxTransform are not valid!"
        return deltas
...
class Box2BoxTransformRotated(object):
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        pred_angle = (pred_angle +
                      180.0) % 360.0 - 180.0  # make it in [-180, 180)
...
def _dense_box_regression_loss(
    ...
    if box_reg_loss_type == "smooth_l1":
        gt_anchor_deltas = [
            box2box_transform.get_deltas(anchors, k) for k in gt_boxes
        ]
        ...
        loss_box_reg = giou_loss(torch.stack(pred_boxes)[fg_mask],
