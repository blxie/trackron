...
class SiameseUT(nn.Module):
    ...
    @configurable
    def __init__(self, *, backbone: nn.Module, sot_head: nn.Module,
                 mot_head: nn.Module, sot_objective: nn.Module,
                 mot_objective: nn.Module, pixel_mean: Tuple[float],
    ...
    def from_config(cls, cfg):
        ...
        sot_head = build_sot_head(cfg.MODEL.SOT,
                                  backbone._out_feature_channels)
        mot_head = build_mot_head(cfg.MODEL.MOT,
                                  backbone._out_feature_channels)
        return {
            "backbone": backbone,
            "sot_head": sot_head,
    ...
    def forward(self, data: List[Dict[str, torch.Tensor]], mode='sot'):
        ...
        if mode == "sot":
            ...
            targets['search_boxes'] = torch.stack(
                [targets['template_boxes'], targets['search_boxes']], dim=1)
            return self.sot_objective(results, targets)
        elif mode == "mot":
            targets = self.prepare_mot_targets(data)
            results = self.mot_head(template_features, search_features,
    ...
    def prepare_sot_targets(self, data):
        targets = {}
        for s in ['template', 'search']:
            targets[f'{s}_boxes'] = torch.stack(
                [d[f'{s}_boxes'] for d in data]).to(self.device)
            targets[f'{s}_images'] = torch.stack(
                [d[f'{s}_images'] for d in data]).to(self.device)
        return targets
    ...
    def prepare_mot_targets(self, data):
        ...
        for idx, d in enumerate(data):
            det_targets += [{
                'boxes':
                norm_boxes(d['template_images'], d['template_boxes']),
                'labels':
                d['template_labels'].to(self.device)
            }]
            track_targets += [{
                'boxes':
                norm_boxes(d['search_images'], d['search_boxes']),
                'labels':
