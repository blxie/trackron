...
class SiameseMOT(nn.Module):
  ...
  def from_config(cls, cfg):
    ...
    track_head = build_mot_head(cfg.MODEL.MOT, backbone._out_feature_channels)
    return {
        "backbone": backbone,
  ...
  def track_mot(self, image, mask=None, ref_info=None):
    ...
    return self.track_head.track(backbone_feat, mask, ref_info)
