...
from trackron.external.deform_conv import ModulatedDeformConv as DCN
from trackron.trackers.utils.postprocessing import mot_decode, _tranpose_and_gather_feat
...
class DeformConv(nn.Module):
    ...
    def __init__(self, chi, cho):
        ...
        self.conv = DCN(chi,
                        ...
                        deformable_groups=1)
        channels_ = 3 * 3 * 3
        self.conv_offset_mask = nn.Conv2d(chi,
                                          channels_,
                                          kernel_size=(3, 3),
                                          stride=1,
                                          padding=1,
...
class DLASeg(nn.Module):
    ...
    def __init__(self,
        ...
        self.base = DLA(levels=levels, channels=channels, block=BasicBlock)
...
def dla34(cfg):
    heads = {
        'hm': cfg.MODEL.NUM_CLASS,
        'wh': 4 if cfg.MODEL.LTRB else 2,
        'id': cfg.MODEL.REID_DIM
    }
    if cfg.MODEL.REG_OFFSET:
        heads['reg'] = 2
    ...
    model = DLASeg(levels,
                   ...
                   last_level=5,
                   head_conv=head_conv,
                   max_detections=cfg.MODEL.MAX_OBJECTS)
    return model
