...
import torch
from trackron.external import PrRoIPool2D
from trackron.models.layers.blocks import LinearBlock
...
class AtomIoUNet(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            input_dim (tuple, optional): [description]. Defaults to (128, 256).
            pred_input_dim (tuple, optional): [description]. Defaults to (256, 256).
            pred_inter_dim (tuple, optional): [description]. Defaults to (256, 256).
            scale (tuple, optional): [description]. Defaults to (1/8, 1/16).
        """
        super().__init__()
        ...
        self.prroi_pool3r = PrRoIPool2D(3, 3, scale[0])
        self.prroi_pool3t = PrRoIPool2D(5, 5, scale[0])
...
class IoUNet(AtomIoUNet):
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        args:
            feat1:  Features from the reference frames (4 or 5 dims).
            feat2:  Features from the test frames (4 or 5 dims).
            bb1:  Target boxes (x,y,w,h) in image coords in the reference samples. Dims (images, 4).
            proposals2:  Proposal boxes for which the IoU will be predicted (images, num_proposals, 4)."""
...
class RegressionNet(nn.Module):
    ...
    def predict_bb(self, modulation, feat, proposals):
        """Predicts box for the give proposals.
        args:
            modulation:  Modulation vectors for the targets. Dims (batch, feature_dim).
            feat:  IoU features (from get_iou_feat) for test images. Dims (batch, feature_dim, H, W).
            proposals:  Proposal boxes for which the IoU will be predicted (batch, num_proposals, 4)."""
