...
def iou_all(reference, proposals):
    ...
    union = reference[2:].prod() + proposals[...,
...
def perturb_box(box, min_iou=0.5, sigma_factor=0.1):
    ...
    perturb_factor = torch.sqrt(box[2] * box[3]) * c_sigma_factor.to(
        box.device)
...
def sample_box_gmm(mean_box,
    ...
    if gt_sigma is None or gt_sigma[0] == 0 and gt_sigma[1] == 0:
        ...
        std_gt = torch.Tensor(
            [gt_sigma[0], gt_sigma[0], gt_sigma[1], gt_sigma[1]]).view(1, 4)
        gt_density = gauss_density_centered(proposals_rel_centered,
                                            std_gt).prod(-1)
...
# TRACED 经过 backbone 之后的 feature map size
...
def generate_proposals(boxes,
    ...
    for box in new_boxes:
        if proposal_method == 'default':
            ...
            proposals, _, _ = sample_box_gmm(box,
                                             iou_thresh,
