...
def tracker_collate_stack0(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        return TensorDict({
            key: tracker_collate_stack0([d[key] for d in batch])
            for key in batch[0]
        })
        ...
        return TensorList(
            [tracker_collate_stack0(samples) for samples in transposed])
    elif isinstance(batch[0], collections.Sequence):
        transposed = zip(*batch)
        return [tracker_collate_stack0(samples) for samples in transposed]
...
def tracker_collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        return TensorDict({
            key: tracker_collate_stack1([d[key] for d in batch])
            for key in batch[0]
        })
    elif isinstance(batch[0], collections.Mapping):
        return {
        ...
        transposed = zip(*batch)
        return TensorList(
            [tracker_collate_stack1(samples) for samples in transposed])
    elif isinstance(batch[0], collections.Sequence):
        transposed = zip(*batch)
        return [tracker_collate_stack1(samples) for samples in transposed]
    elif batch[0] is None:
        return batch
...
class TrackingLoader(torch.utils.data.dataloader.DataLoader):
    ...
    def __init__(self,
        ...
        if collate_fn is None:
            if stack_dim == 0:
                collate_fn = tracker_collate_stack0
            elif stack_dim == 1:
                collate_fn = tracker_collate_stack1
            else:
                raise ValueError('Stack dim no supported. Must be 0 or 1.')

        super(TrackingLoader,
