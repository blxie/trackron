...
class TAODataset(BaseDataset):
    ...
    def __init__(self, data_root: Path, split='train', mode='mot'):
        super().__init__()
    ...
    def _construct_sequence(self, vid):
        ...
        frames_list = list(
            sorted([str(frame) for frame in frames_path.glob('*.jpg')]))
        min_index = min(
        ...
        for frame_idx, frame_name in enumerate(frames_list):
            ...
            if self.has_gt:
                init_data[frame_idx]['annotations'] = self.img_ann_map[
                    image_id]
...
def load_tao_json(json_file,
    ...
    #   'id': 42986},
    #  ...]
    anns = [[tao_api.img_ann_map[img['id']] for img in imgs]
            for imgs in video_imgs]
    ...
    if not json_file.match("*minival*"):
        ...
        ann_ids = [
            ann["id"] for img_ann in chain.from_iterable(anns)
            for ann in img_ann
        ]
    ...
    logger.info(
        "Loaded {} videos, containing {} frames, in TAO format from {}".format(
            len(videos), num_imgs, json_file))

    dataset_dicts = []

    ann_keys = ["bbox", "category_id", "track_id"] + (extra_annotation_keys
                                                      or [])

    num_instances_without_valid_segmentation = 0

    for (video_dict, img_dict_list,
         anno_dict_list) in zip(videos, video_imgs, anns):
        record = {}
        ...
        for img_dict, anno_dict_list in zip(img_dict_list, anno_dict_list):
            ...
            assert img_dict[
                'video_id'] == video_id, "video id should be the same"
...
def convert_to_tao_dict(dataset_name):
    ...
    for image_id, image_dict in enumerate(dataset_dicts):
        ...
        for annotation in anns_per_image:
            ...
            to_bbox_mode = BoxMode.XYWH_ABS if len(
                bbox) == 4 else BoxMode.XYWHA_ABS
            ...
            if "segmentation" in annotation:
                ...
                if to_bbox_mode == BoxMode.XYWH_ABS:
                    bbox_xy = BoxMode.convert(bbox, to_bbox_mode,
                                              BoxMode.XYXY_ABS)
            ...
            if "segmentation" in annotation:
                seg = tao_annotation["segmentation"] = annotation[
                    "segmentation"]
