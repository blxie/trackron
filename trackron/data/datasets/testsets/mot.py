...
class MOTDataset(BaseDataset):
    ...
    def init_public_detections(self,
                               data,
                               public_detections,
                               start_frame_id=0,
                               type='fairmot'):
        ...
        for frame_id in sorted(frame_dets.keys()):
            ...
            if frame_idx - last_frame_id > 1:
                ...
                for idx in range(last_frame_id + 1, frame_id):
                    data[idx]['detections'] = np.array([[0, 0, 1.0, 1.0,
                                                         -1.0]])
    ...
    def _construct_sequence(self, vid):
        ...
        frames_list = list(
            sorted([
                str(self.base_path / f['file_name'])
                for f in self.vid_img_map[vid]
            ]))
        ...
        if self.has_gt:
            ...
            for track in self.vid_track_map[vid]:
                ...
                frame_idx = self.imgs[
                    ann['image_id']]['frame_index'] - min_index
