...
def get_trainsets(dataset_names, data_dir, image_loader=opencv_loader):
    ...
    for name in dataset_names:
        ...
        if name == "lasot":
            trainsets.append(
                # TRACED
                Lasot(data_dir / 'LaSOT/LaSOTBenchmark',
                      split='train',
                      image_loader=image_loader))
        elif name == "got10k_vottrain":
            trainsets.append(
                # TRACED
                Got10k(data_dir / 'GOT-10k' / 'train',
                       split='vottrain',
                       image_loader=image_loader))
            ...
            trainsets.append(
                TAO(data_dir / 'TAO', split='train',
                    image_loader=image_loader))
        elif name == "mot":
            trainsets.append(
                MOT(data_dir / 'MOT', split='train',
                    image_loader=image_loader))
        elif name == "mots":
            trainsets.append(
                MOT(data_dir / 'MOT',
            ...
            trainsets.append(
                LVISSeq(data_dir / 'coco',
                        split='train',
                        image_loader=image_loader))
        elif name == "crowdhuman":
            trainsets.append(
                CrowdHumanSeq(data_dir / 'CrowdHuman',
                              image_loader=image_loader))
        elif name == "vid":
            trainsets.append(
                ImagenetVID(data_dir / 'VID', image_loader=image_loader))
