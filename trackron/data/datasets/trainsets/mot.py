...
class MOT(BaseVideoDataset):
  ...
  def __init__(self,
    ...
    """
    super().__init__('MOT', root, image_loader)

    split_root = 'train' if split != 'test' else 'test'
    self.img_pth = os.path.join(root, version, split_root)
    self.anno_path = os.path.join(root, version, 'annotations/{}.json'.format(split))
    ...
    for fid in frame_ids:
      ...
      for ann in anno[fid]:
        ...
        if 'segmentation' in ann:
          ann['mask'] = coco_mask.decode(ann['segmentation'])
        frame_annos += [ann]
      annos += [frame_annos]
