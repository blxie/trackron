...
class StarkDataset(torch.utils.data.Dataset):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'interval':
                    ...
                    # Sample test and train frames in a casual manner, i.e. search_frame_ids > template_frame_ids
                    while search_frame_ids is None:
            ...
            try:
                ...
                template_masks = template_anno[
                    'mask'] if 'mask' in template_anno else [
                        torch.zeros((H, W))
                    ] * self.num_template_frames
                search_masks = search_anno[
                    'mask'] if 'mask' in search_anno else [
                        torch.zeros((H, W))
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            ) > 2 * (self.num_search_frames +
                     self.num_template_frames) and len(visible) >= 20
    ...
    def get_frame_ids_trident(self, visible):
        ...
        while None in template_frame_ids_extra or len(
                template_frame_ids_extra) == 0:
            template_frame_ids_extra = []
            # first randomly sample two frames from a video
            template_frame_id1 = self._sample_visible_ids(
                visible, num_ids=1)  # the initial template id
            search_frame_ids = self._sample_visible_ids(
                visible, num_ids=1)  # the search region id
            # get the dynamic template id
            # for max_gap in self.max_gap:
            max_gap = self.max_gap
            if template_frame_id1[0] >= search_frame_ids[0]:
                min_id, max_id = search_frame_ids[
                    0], search_frame_ids[0] + max_gap
            else:
                min_id, max_id = search_frame_ids[
                    0] - max_gap, search_frame_ids[0]
            if self.frame_sample_mode == "trident_pro":
                f_id = self._sample_visible_ids(visible,
                                                num_ids=1,
                                                min_id=min_id,
                                                max_id=max_id,
                                                allow_invisible=True)
