...
class MOTProcessing(SiameseBaseProcessing):
    ...
    def _valid_filter(self,
        ...
        for idx, (trackids, boxes, labels) in enumerate(
                zip(trackids_list, boxes_list, labels_list)):
    ...
    def __call__(self, data: TensorDict):
        ...
        share_track_ids = set(
            [ann['track_id'] for ann in data['template_anno'][0]])
        for s in ['template', 'search']:
            ...
            trackids_list = [
                torch.tensor([ann['track_id'] for ann in anns],
                             dtype=torch.long) for anns in annotations
            ]
            boxes_list = [
                torch.tensor([ann['bbox'] for ann in anns],
                             dtype=torch.float32) for anns in annotations
            ]
            labels_list = [
                torch.tensor([ann['category_id'] for ann in anns],
                             dtype=torch.long) for anns in annotations
            ]
            if 'mask' in annotations[0][0]:
                masks_list = [
                    torch.tensor([ann['mask'] for ann in anns], dtype=bool)
            ...
            if masks_list is not None:
                ...
                images_list, boxes_list = self.transform['joint'](
                    image=images_list,
                    bbox=boxes_list,
                    joint=True,
                    new_roll=new_roll)
            ...
            data[f'{s}_trackids'], data[f'{s}_boxes'], data[
                f'{s}_labels'], data[f'{s}_masks'] = self._valid_filter(
                    trackids_list,
                    boxes_list,
                    labels_list,
                    image_sz,
            ...
            for trackids in data[f'{s}_trackids']:
                share_track_ids = share_track_ids.intersection(
                    set(trackids.tolist()))
            data[f'{s}_id_index'] = [{
                trackid.item(): idx
        ...
        inds_list = [
            torch.tensor([id_index[track_id] for track_id in share_track_ids],
                         dtype=torch.long)
            for id_index in data['search_id_index']
        ]
        data['search_track_labels'] = [
            labels[inds]
            for labels, inds in zip(data['search_labels'], inds_list)
        ]
        ...
        if data['search_masks'] is not None:
            data['search_track_masks'] = [
                masks[inds]
                for masks, inds in zip(data['search_masks'], inds_list)
            ]
        ...
        image = (data['search_images'][0].permute(1, 2, 0) *
        ...
        if self.mode == 'sequence':
            ...
            data = data.apply(lambda x: x[0] if isinstance(
                x, (list, tuple, torch.Tensor)) else x)
...
class MOTProcessingMultiClass(MOTProcessing):
    ...
    def get_search_box(self, image_sz, jitter_box, search_boxes):
        ...
        search_boxes = torch.stack([x1 - tx, y1 - ty, x2 - tx, y2 - ty],
                                   dim=-1)
    ...
    def _valid_filter(self, trackids_list, boxes_list, labels_list, image_sz):
        new_trackids_list, new_box_list, new_label_list = [], [], []
        height, width = image_sz
        for trackids, boxes, labels in zip(trackids_list, boxes_list,
                                           labels_list):
            boxes = box_convert(boxes, 'xywh', 'xyxy')
            valid = (boxes[:, 0] <= width) & (boxes[:, 1] <= height) & (
                boxes[:, 2] > 0) & (boxes[:, 3] > 0) & (
