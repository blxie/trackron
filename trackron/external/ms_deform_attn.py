...
class MSDeformAttnFunction(Function):
    ...
    @staticmethod
    def forward(ctx, value, value_spatial_shapes, sampling_locations,
                attention_weights, im2col_step):
        ctx.im2col_step = im2col_step
        output = OPS.ms_deform_attn_forward(value, value_spatial_shapes,
                                            sampling_locations,
                                            attention_weights, ctx.im2col_step)
        ctx.save_for_backward(value, value_spatial_shapes, sampling_locations,
                              attention_weights)
        return output
...
def ms_deform_attn_core_pytorch_mot(query,
                                    value,
    ...
    v = torch.stack(sampling_value_list,
                    dim=-2).flatten(-2)  # (N_*M_, D_, Lq_, L_*P_)
    k = key_proj(v.reshape(N_, M_ * D_,
                           Lq_, L_ * P_).permute(0, 2, 3, 1)).permute(
                               0, 3, 1, 2).reshape(N_ * M_, D_, Lq_, L_ * P_)
