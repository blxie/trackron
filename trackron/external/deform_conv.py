...
# from trackron.utils.develop import create_dummy_class, create_dummy_func
...
class _DeformConv(Function):
    ...
    def _cal_im2col_step(input_size, default_size):
        ...
        if input_size <= default_size:
            return input_size
        best_step = 1
        for step in range(2, min(int(math.sqrt(input_size)) + 1,
                                 default_size)):
...
try:
    from trackron import _C
except ImportError:
    # TODO: register ops natively so there is no need to import _C.
    _msg = "trackron is not compiled successfully, please build following the instructions!"
    _args = ("trackron._C", _msg)
